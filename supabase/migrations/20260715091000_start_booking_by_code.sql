-- ─────────────────────────────────────────────────────────────────────────
-- Redefine start_booking to accept the trip CODE directly (text) rather than a
-- trip_code_id, so the client flow (which only carries the code) can start a
-- booking without ever seeing the hidden trip_codes table. The code is resolved
-- + validated inside the function. Behaviour is otherwise unchanged.
-- ─────────────────────────────────────────────────────────────────────────
